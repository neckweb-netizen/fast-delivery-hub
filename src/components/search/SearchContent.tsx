...
export const SearchContent = () => {
  ...
  const renderCategoryIcon = (categoria: any) => {
    ...
    if (categoria.icone.match(/[\u{1F000}-\u{1F9FF}]|[\u{2600}-\u{27BF}]|[\u{1F300}-\u{1F5FF}]|[\u{1F600}-\u{1F64F}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{FE00}-\u{FE0F}]|[\u{E0020}-\u{E007F}]|[\u{200D}]|[\u{20E3}]|[\u{2700}-\u{27BF}]|[\u{24C2}]|[\u{1F170}-\u{1F251}]/u)) {
      return <span className="text-xs">{categoria.icone}</span>;
    }

    return <img src={categoria.icone} alt="" className="w-3 h-3 object-contain" loading="lazy" />;
  };
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {categorias?.map(categoria => (
                <Badge key={categoria.id} variant={selectedCategory === categoria.id ? "default" : "outline"} className="cursor-pointer flex items-center gap-1" onClick={() => setSelectedCategory(categoria.id)}>
          ...
          {filteredEmpresas.length === 0 ? (
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredEmpresas.map((empresa: any) => (
                <LocalCard key={empresa.id} empresa={empresa} />
              ))}
            </div>
          )}
        </>
